...
""".strip()
    ...
    days, avg_add, avg_del, avg_net, avg_files, avg_churn, avg_commits = (
        get_non_zero_day_averages()
    )
    if days == 0:
        ...
        churn_ratio = (
            avg_churn / (avg_add + avg_del) * 100 if (avg_add + avg_del) > 0 else 0.0
        )
        print(
            f"days: {days}\n"
            f"repos: {repo_count}\n"
...
def get_ticket_counts(repo_path):
    ...
    archive_dir = os.path.join(tickets_dir, "archive")
        ...
        len(glob.glob(os.path.join(archive_dir, "T-*.md")))
